...
## [1.17.2] — 2026-07-21 — F-14B(U) real footprint (second survey)

### Added — surveyed airframe dimensions (`airframe_dimensions.json`)

A second survey (exporter fired this time) returned the F-14B(U)'s real in-sim
bounding box: **length 19.62 m, span 20.34 m, height 6.29 m**. pydcs models the
F-14 span as only 10.15 m (swept/stored), so the occupancy registry understated
the parked footprint by ~1.9 m and could spawn GSE/statics under a Tomcat's
spread wingtips.

- New `airframe_dimensions.json` override pack, consulted by the occupancy
  `_half_extent` when pydcs understates a type. Applied to **F-14BU only**
  (half-extent 8.3 m → 10.17 m). Scoped to the new module so no existing share
  link changes bytes — determinism intact.
- Re-confirmed from the same survey: type id **`F-14BU`**, and the Stennis in the
  scene reports `catapult / ACLS / Arresting Gear`, consistent with the B(U)
  catobar carrier setup.

**Open decision (Rob):** the survey shows the *released* F-14B is also 20.34 m
span in-sim (pydcs: 10.15 m) — the same understatement affects existing F-14B
static placement. Correcting it changes generated bytes for any mission with
parked F-14Bs (share-link determinism), so it's a deliberate versioned data pass,
not folded in here. `_F-14B_observed` is recorded in the pack for when we do it.
