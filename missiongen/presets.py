...
def apply(group, rows, guard_mhz):
    """Program radio 1 presets on every unit of the player/client group.

    Modules without ME-settable radios (no panel_radio template) are skipped
    silently — generation never fails over a preset. Returns True if at least
    one unit was programmed."""
    programmed = False
    for u in group.units:
        try:
            ...
            continue
        radio1 = (u.radio or {}).get(1)
        channels = radio1.get("channels") if isinstance(radio1, dict) else None
        if not channels:
            continue
        for ch, _agency, mhz in rows:
            if ch in channels:
                channels[ch] = mhz
        channels[max(channels)] = guard_mhz    # Guard on the last channel, per SOP
        programmed = True
    return programmed
