...
def _airframe_dims():
    ...
    _comment/_source/_observed underscore keys)."""
...
def dress_airfield(m, airport, country, era_side_cfg, density, rng: random.Random,
    ...
    def _half_extent(unit_type):
        ...
        Prefers a surveyed real bounding box (airframe_dimensions.json) where
        pydcs understates the envelope — e.g. pydcs gives the F-14 a 10.15 m span
        (swept/stored) while its real spread envelope is 20.34 m, which let GSE
        spawn under the wingtips. Falls back to pydcs width/length otherwise."""
